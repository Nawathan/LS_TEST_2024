...
<?php
include "./sidebar_responsive_tum.php";
?>
<!DOCTYPE html>
<html>
...
<script>
    ...
    function callTicketStatus(buttonStatus) {
        ...
        switch (buttonStatus) {
            case 'jan':
                start_date = (today.format('YYYY') + '-01-01');
                end_date = (today.format('YYYY') + '-01-31');
                title = "กลุ่มบริษัทเดือนมกราคม";
                break;
            case 'feb':
                start_date = (today.format('YYYY') + '-02-01');
                end_date = (today.format('YYYY') + '-02-28');
                title = "กลุ่มบริษัทเดือนกุมภาพันธ์";
                break;
            case 'mar':
                start_date = (today.format('YYYY') + '-03-01');
                end_date = (today.format('YYYY') + '-03-31');
                title = "กลุ่มบริษัทเดือนมีนาคม";
                break;
            case 'apr':
                start_date = (today.format('YYYY') + '-04-01');
                end_date = (today.format('YYYY') + '-04-30');
                title = "กลุ่มบริษัทเดือนเมษายน";
                break;
            case 'may':
                start_date = (today.format('YYYY') + '-05-01');
                end_date = (today.format('YYYY') + '-05-31');
                title = "กลุ่มบริษัทเดือนพฤษภาคม";
                break;
            case 'jun':
                start_date = (today.format('YYYY') + '-06-01');
                end_date = (today.format('YYYY') + '-06-30');
                title = "กลุ่มบริษัทเดือนมิถุนายน";
                break;
            case 'jul':
                start_date = (today.format('YYYY') + '-07-01');
                end_date = (today.format('YYYY') + '-07-31');
                title = "กลุ่มบริษัทเดือนกรกฎาคม";
                break;
            case 'aug':
                start_date = (today.format('YYYY') + '-08-01');
                end_date = (today.format('YYYY') + '-08-31');
                title = "กลุ่มบริษัทเดือนสิงหาคม";
                break;
            case 'sep':
                start_date = (today.format('YYYY') + '-09-01');
                end_date = (today.format('YYYY') + '-09-30');
                title = "กลุ่มบริษัทเดือนกันยายน";
                break;
            case 'oct':
                start_date = (today.format('YYYY') + '-10-01');
                end_date = (today.format('YYYY') + '-10-31');
                title = "กลุ่มบริษัทเดือนตุลาคม";
                break;
            case 'nov':
                start_date = (today.format('YYYY') + '-11-01');
                end_date = (today.format('YYYY') + '-11-30');
                title = "กลุ่มบริษัทเดือนพฤศจิกายน";
                break;
            case 'dec':
                start_date = (today.format('YYYY') + '-12-01');
                end_date = (today.format('YYYY') + '-12-31');
                title = "กลุ่มบริษัทเดือนธันวาคม";
                break;
            case 'all':
                start_date = (today.format('YYYY') + '-01-01');
                end_date = (today.format('YYYY') + '-12-31');
                title = "กลุ่มบริษัทปี " + $('#year').val();
                break;
        }
    ...
    $.fn.get_ticket = function(startOfMonth, endOfMonth) {
        ...
        $.post('./personal/get_graph.php', {
            ...
            if (data.length > 0) {
                var arr_data = [
                    ['Date', 'Complete', 'Pending', 'Open', 'Close']
                ];
    ...
    $.fn.get_status_quotation = function(startOfMonth, endOfMonth) {
        ...
        $.post('./personal/get_graph.php', {
            ...
            if (data.length > 0) {
                ...
                var arr_data = [
                    ['Date', 'Lost', '10% Pipeline', '30% POC', '50% Quotation', '80% Final Quotation', '100% WIN']
                ];
                $.each(data, function(index, arr) {
    ...
    $.post('/user/get_user.php', {
        event: "get_all_type_permission_by_id",
